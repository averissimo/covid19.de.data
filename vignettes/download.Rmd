...
my.rki %>% 
  ...
  ggplot(aes(x = date, y = cases, color = state)) + 
    ...
    labs(title = 'New deaths per day in last 12 days', y = 'Deaths', x = 'Day',
         subtitle = 'Showing only 6 most affected states')
```
...
```{r}
my.rki %>% 
  filter(district %in% (top.districts %>% head(7))) %>%
  filter(anydate(base::date()) - date <= 12) %>% 
  arrange(desc(date)) %>% 
  ...
  ggplot(aes(x = date, y = cases, color = district)) + 
    ...
    theme(legend.position = 'none') +
    labs(title = 'New cases per day in last 12 days', y = 'Cases', x = 'Day',
         subtitle = 'Showing only 7 most affected states')
```
...
my.rki %>% 
  ...
  ggplot(aes(x = date, y = cases, color = district)) + 
    ...
    theme(legend.position = 'none') +
    labs(title = 'New cases per day in last 12 days', y = 'Cases', x = 'Day',
         subtitle = 'Showing only 7 most affected districts')
```
...
```{r}
my.rki %>% 
  filter(state %in% (top.states %>% head(7))) %>%
  filter(anydate(base::date()) - date <= 12) %>% 
  arrange(date) %>% 
  group_by(state, date) %>%
  summarize(cases = sum(cases)) %>%
